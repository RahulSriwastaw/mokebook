...
export function JEELayout(props: JEELayoutProps) {
    ...
    const sections = useMemo(() => {
        const map = new Map<string, { total: number; answered: number; marks: number }>();
        questions.forEach((q) => {
            const sec = q.section || "General";
            if (!map.has(sec)) map.set(sec, { total: 0, answered: 0, marks: 0 });
            const entry = map.get(sec)!;
            entry.total++;
            entry.marks += q.marks || 1;
            if (answers[q.id]?.length) entry.answered++;
        });
    ...
    const typeColor = typeColors[qType] || "#6B7280";

    const paletteQuestions: Array<{ id: string; number: number; section?: string; type?: string; status: any }> = filteredQuestions.map((q, idx) => {
        ...
        return { id: q.id, number: q.number || idx + 1, section: q.section, type: q.type, status };
    });

    return (
        <div className="h-screen flex flex-col" style={{ ...cssVariables, background: "var(--exam-bg)" }}>
            {/* Section tabs for Physics/Chem/Math */}
            <div className="h-10 flex items-center gap-1 px-2 border-b overflow-x-auto shrink-0" style={{ background: "var(--exam-header-bg)", borderColor: "var(--divider)" }}>
                <button onClick={() => setActiveSection("all")} className={cn("px-3 h-7 rounded text-[11px] font-semibold whitespace-nowrap", activeSection === "all" ? "text-white" : "text-[var(--text-secondary)] hover:bg-[var(--bg-main)]")} style={{ background: activeSection === "all" ? "var(--exam-primary)" : undefined }}>All</button>
                {sections.map((sec) => (
                    <button key={sec.name} onClick={() => setActiveSection(sec.name)} className={cn("px-3 h-7 rounded text-[11px] font-semibold whitespace-nowrap", activeSection === sec.name ? "text-white" : "text-[var(--text-secondary)] hover:bg-[var(--bg-main)]")} style={{ background: activeSection === sec.name ? "var(--exam-primary)" : undefined }}>
            ...
            <header className="h-12 flex items-center justify-between px-4 border-b shrink-0" style={{ background: "var(--exam-header-bg)", borderColor: "var(--divider)" }}>
                <div className="flex items-center gap-2">
                    ...
                    {/* Question type badge */}
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded-full text-white" style={{ background: typeColor }}>
                        {qType === "mcq_single" ? "Single Correct" : qType === "mcq_multiple" ? "Multi Correct" : "Integer"}
                    </span>
                </div>
            ...
            <div className="flex-1 flex overflow-hidden">
                <div className="flex-1 min-w-0 overflow-y-auto p-4 lg:p-6">
                    {question && (
                        <div className="max-w-4xl mx-auto space-y-4">
                            <QuestionDisplay question={{ ...question, number: question.number || currentIndex + 1 }} showType={true} />
                            <OptionButton options={question.options || []} type={qType} selected={selected} onChange={(sel) => onAnswer(question.id, sel)} integerValue={integerAnswers[question.id]} onIntegerChange={(val) => onIntegerChange(question.id, val)} />
                        </div>
                    )}
                </div>
                <aside className="w-[260px] lg:w-[300px] border-l hidden md:flex flex-col p-4 overflow-hidden" style={{ background: "var(--exam-sidebar-bg)", borderColor: "var(--divider)" }}>
                    ...
                    <div className="mt-3 pt-3 border-t border-[var(--divider)] space-y-1.5">
                        <p className="text-[10px] font-bold uppercase text-[var(--text-muted)]">Marks Summary</p>
                        {sections.map((sec) => (
                            <div key={sec.name} className="flex justify-between text-[11px]">
                                <span className="text-[var(--text-secondary)]">{sec.name}</span>
                                <span className="font-semibold text-[var(--text-primary)]">{sec.answered}/{sec.total}</span>
                            </div>
                        ))}
                    </div>
                </aside>
            ...
            <div className="h-14 flex items-center justify-between px-4 border-t shrink-0" style={{ borderColor: "var(--divider)" }}>
                <div className="flex items-center gap-2">
                    <Button variant="outline" size="sm" onClick={onMarkReview} className={cn("h-8 text-[12px]", isMarked && "border-pink-400 text-pink-600 bg-pink-50")}>
